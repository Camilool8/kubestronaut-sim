...
has_name "$names" rotate || {
  ...
  evidence "The container is found by name, and the question names it rotate. kubectl create cronjob names the container after the CronJob instead, so a generated manifest has to be renamed — everything else about the template is read off that name and is beside the point while no container carries it."
  exit 1
}
