...
snapshot() {
  kubectl -n atlas get pod tuned -o json 2>/dev/null | jq -S --arg c web '
    {
      volume: ((first(.spec.volumes[]? | select(.name == "limits")) | {configMap: {name: (.configMap.name // null)}}) // null),
      mount: ((first(.spec.containers[]? | select(.name == $c) | .volumeMounts[]? | select(.name == "limits"))
               | {mountPath: (.mountPath // null), readOnly: (.readOnly // null)}) // null)
    }' 2>/dev/null
}
...
has_name "$vols" limits || {
  ...
  show_actual text "volumes that exist: $(name_list "$vols")"
  show_why "A volume's name is the handle its mount refers to, and the question fixes it at 'limits'. Naming the volume after the ConfigMap it carries is the common habit and works perfectly at runtime, but this check looks the volume up by the name it was told to expect, so a different one is invisible to it."
  exit 1
}
