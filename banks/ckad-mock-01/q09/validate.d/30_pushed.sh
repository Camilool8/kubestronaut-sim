...
# expected: none — the check grades whether a push actually reached the
#           registry, read back as tag presence in the registry's own API
#           response. That is an event rather than a document the candidate
#           authored; the tags/catalog pane above already names what the
#           registry holds.
set -uo pipefail
