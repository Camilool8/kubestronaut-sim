...
# Only the authored half of this check — the claim's own spec — gets a
# generated document. Whether it actually bound, and to which volume, is a
# live relationship between two objects, not a shape the candidate wrote; its
# outcome rides on the crit messages below (both already name the reading)
# rather than a second pane.
snapshot() {
  jq -n -S \
    --arg size "${size:-}" --arg modes "${modes:-}" --arg class "${class:-}" \
    '{
      requests: {storage: (if $size == "" then null else $size end)},
...
crit 1 "is Bound" \
  ...
  "A claim stays Pending until some volume satisfies ALL THREE of size, access mode and class." \
  -- [ "$phase" = "Bound" ]
