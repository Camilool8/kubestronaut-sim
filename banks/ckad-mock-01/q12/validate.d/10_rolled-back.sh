...
[ "$img" = "nginx:1.27-alpine" ] || {
  echo "image is '$img', want nginx:1.27-alpine"
...
[ "$spec" = "4" ] || {
  echo "spec.replicas is '$spec', want 4"
  evidence "Scaling is not part of a revision, so rolling back does not undo it — the replica count you set survives the rollback untouched. Expecting an undo to restore the old count is one of the more expensive surprises this object has to offer."
  exit 1
}
[ "$ready" = "4" ] && echo "rolled back, 4/4 ready" || {
  echo "readyReplicas is '$ready', want 4"
  evidence "spec.replicas is what was asked for; status.readyReplicas is what the cluster actually has. The gap is Pods still starting, or Pods that cannot start — a rollout that has not finished settling reads exactly like this for a few seconds."
  exit 1
}
