...
# expected: replicasets-while-paused.txt text
set -uo pipefail
. /banks/_lib/checks.sh

snapshot() {
  cat /opt/course/31/replicasets-while-paused 2>/dev/null
}

evidence() {
  show_pair text replicasets-while-paused.txt
  show_why "$1"
}
...
[ "$got" = "1" ] && echo "the paused ReplicaSet count was recorded" || {
  ...
  evidence "This number is the evidence that pausing did anything, and it can only be read in the window the question describes: after the image was changed and before the rollout was resumed. There is one ReplicaSet then, because the new Pod template was stored and no controller acted on it. Two means the image was changed before the pause — the rollout had already started and there was nothing left to hold back. An empty file means the count was never taken; resuming first destroys the answer, since the second ReplicaSet exists from that moment on."
  exit 1
}
