...
Every row here is something `./sim doctor` reports (`sim`). Run it
...
| `python3 : MISSING (./sim up needs it)` | `./sim` reads JSON with `python3` (`sim`) | Install `python3`. Without it `up` prints no phase lines and spins until `SIM_BOOT_BUDGET` expires rather than finishing |
...
| Symptom | Cause | Fix |
|---|---|---|
| `The environment failed to start:` then an error | `/api/boot` reported `failed` (`sim`) | Read the printed error, then `docker compose logs k8s-env` for the rest |
| `Gave up waiting after 3600s.` | The boot budget elapsed. The environment may still be working (`sim`) | `docker compose logs -f k8s-env` to see whether it is progressing. Raise it with `SIM_BOOT_BUDGET=<seconds> ./sim up` |
...
| Building an image on an instance is slow and disk-hungry | Podman on the instances runs the `vfs` storage driver. An instance is itself a container, so stacking overlayfs on overlayfs needs fuse-overlayfs; vfs copies each layer in full instead, which is slower on large images but has fewer moving parts. The questions' images are tiny enough that it measured the same | Expected |
| `podman run` reports a cgroup controller is unavailable | Podman is one level below a cgroup hierarchy it does not own, so `images/instance/containers.conf` disables cgroup creation. That costs podman's ability to limit a container's resources, which no question asks for | Expected. Plain `podman run` works as typed |
...
| `Reset failed: <error>` | The conductor's reset job did not settle cleanly (`sim`) | Read the error, then `docker compose logs conductor`. `./sim purge && ./sim up` is the fallback |
