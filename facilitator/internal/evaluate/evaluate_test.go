...
func TestSSHArgs(t *testing.T) {
	got := sshArgs("/shared/ssh/id_ed25519", "/tmp/ssh-mux/%C", "instance-1", "echo hi")
	want := []string{
...
func TestSSHArgsWithoutAControlPathAreUnchanged(t *testing.T) {
...
// an ordinary connection, so a check would score zero. Losing the directory has
// to cost multiplexing, not the grade.
func TestControlPathIsEmptyWhenTheDirectoryCannotBeMade(t *testing.T) {
	...
	if got := controlPath(filepath.Join(blocked, "ssh-mux")); got != "" {
		t.Errorf("controlPath = %q, want \"\" so sshArgs leaves multiplexing out", got)
	}
